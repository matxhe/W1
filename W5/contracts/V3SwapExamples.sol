...
// import '../contracts/interfaces/v3/v3-core/TransferHelper.sol';
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import '../contracts/interfaces/V3/ISwapRouter.sol';

contract V3SwapExamples {
    ...
    function swapExactOutputSingle(uint256 amountOut, uint256 amountInMaximum) external returns (uint256 amountIn) {
        ...
        IERC20(DAI).safeTransferFrom(msg.sender, address(this), amountInMaximum);

        // Approve the router to spend the specified `amountInMaximum` of DAI.
        // In production, you should choose the maximum amount to spend based on oracles or other data sources to achieve a better swap.
        // TransferHelper.safeApprove(DAI, address(swapRouter), amountInMaximum);
        IERC20(DAI).safeApprove(address(swapRouter), amountInMaximum);
        ...
        if (amountIn < amountInMaximum) {
            ...
            IERC20(DAI).safeApprove(address(swapRouter), 0);
            // TransferHelper.safeTransfer(DAI, msg.sender, amountInMaximum - amountIn);
            IERC20(DAI).safeTransfer(msg.sender, amountInMaximum - amountIn);
        }
    }
    ...
    function swapExactOutputMultihop(uint256 amountOut, uint256 amountInMaximum) external returns (uint256 amountIn) {
        ...
        if (amountIn < amountInMaximum) {
            ...
            IERC20(DAI).safeTransferFrom(address(this), msg.sender, amountInMaximum - amountIn);
        }
    }
}